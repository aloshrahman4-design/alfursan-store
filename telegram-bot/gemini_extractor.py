...
class ProductData(BaseModel):
    ...
    price_is_pack_total: bool = Field(
        default=False,
        description=(
            "True عندما لا يوجد أي تسمية درزن/مفرد إطلاقاً، والرقم المستخرج في dozen_price "
            "هو في الحقيقة السعر الإجمالي للعبوة كاملة (مثال: تنسيق 'X زوج Y,Z الف')"
        ),
    )
...
4) الأسعار -- قد تظهر بأشكال مختلفة جداً، ميّز بينها بهذا الترتيب من القواعد:
   ...
   g. أحياناً يظهر رقم أو رمز غريب منفصل ملاصق للسعر وليس جزءاً منه فعلياً (مثل رقم
      عشوائي مطبوع بالخطأ من قالب المورد، غير مسبوق أو متبوع بأي وحدة أو تسمية منطقية) --
      لا تُدرج هذا الرقم الغريب ضمن قيمة السعر الرقمية نفسها (القاعدة (5) أدناه توضح كيف
      تتعامل معه بصريًا).
   h. أحياناً لا توجد أي تسمية "درزن"/"مفرد" ولا حتى كلمة "التعبئة" -- بدلاً من ذلك يظهر
      سطر واحد غامق يجمع الكمية والسعر معاً بصيغة "<كمية> زوج <سعر>,<كسر> الف"، مثال:
      "24 زوج 61,5 الف" أو "18 زوج 66,5 الف". هنا: الفاصلة "," ليست فاصلة آلاف بل فاصلة
      عشرية، وكلمة "الف" تعني ضرب الرقم بالكامل ×1000 -- أي أن "61,5 الف" = 61500،
      و"66,5 الف" = 66500 (اكتب الرقم الصحيح الكامل 61500 أو 66500 في dozen_price، وليس
      61.5). ضع رقم الكمية فقط (بدون كلمة "زوج") في pack_quantity_raw. هذا الرقم الواحد
      هو السعر الإجمالي للعبوة/الكرتونة كاملةً -- وليس سعر درزن حقيقياً ولا سعر قطعة --
      لذا اضبط price_is_pack_total=true، واترك single_price فارغاً (null).

5) لكل سعر وجدته (dozen_price و/أو single_price)، حدّد أيضاً صندوق إحاطة (bounding box)
   حول أرقام ذلك السعر -- بدون رمز العملة، بدون كلمة الوصف المجاورة له -- بمقياس 0 إلى
   1000 نسبةً لأبعاد الصورة الكاملة، بالترتيب: y_min (الحافة العلوية)، x_min (الحافة
   اليسرى)، y_max (الحافة السفلية)، x_max (الحافة اليمنى). ضعه في dozen_price_bbox أو
   single_price_bbox المقابل.

   مهم جداً: إن كان الرقم محاطاً بقوسين "(" و ")"، أو وُجد رقم غريب منفصل بجانبه (كما في
   القاعدة 4g) على نفس السطر -- وسِّع حدود الصندوق ليشمل ذلك أيضاً، حتى لو كانت هناك
   مسافة صغيرة واضحة تفصله عن رقم السعر نفسه (وليس فقط عندما يكون ملاصقاً بلا مسافة) --
   لأن كل هذا سيُمحى ويُستبدل بالرقم الجديد نظيفاً عند التعديل، ولا داعي لإبقاء أي جزء
   منه ظاهراً في الصورة المعدَّلة (لا تُدرج القوس أو الرقم الغريب كجزء من القيمة الرقمية
   نفسها بالطبع). لا يشمل هذا التوسيع رمز العملة (دينار) ولا كلمة الوصف (درزن/مفرد/قطعة)
   المجاورة -- تلك تبقى خارج الصندوق كما في القاعدة الأساسية أعلاه.

   إن لم تستطع تحديد الموقع بدقة كافية، اترك حقل الـ bbox فارغاً حتى لو أعطيت الرقم نفسه
   في الحقل الرقمي.
"""
